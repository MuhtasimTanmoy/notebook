...
- This process gets around even symmetric `NAT`s because both the client and peer can at least talk to the `TURN` server, which has allocated a relay IP address for communication.

- While `TURN` is more robust than `STUN` in that it assist in traversal of more types of NATs. 
...
- In order to exchange media, WebRTC uses session description protocol (SDP) to initiate and execute an `offer` and `answer` mechanism between endpoints or peers. 

- Supported codecs, connectivity, and protocols are added to the SDP so that clients can decide what media codecs `Session Traversal Utilities for NAT` (STUN) allows clients to learn what their public NAT’d IP address and port is. 
